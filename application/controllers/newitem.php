...
class Newitem extends CI_Controller
{
    ...
    public function lihat_item_paging($offset = 0)
    {
        ...
        $config = array(
            'base_url' => 'newitem/lihat_item_paging',
    ...
    public function proses_item()
    {
        ...
        if($method == 'create') {
            ...
            $nip = $this->item_model->insert_item($data);
            $data['nip'] = $nip;
            $item = $data;
        } else {
            $data['nama_pegawai'] = $this->input->post('nama_pegawai');
            ...
            $this->item_model->update_item($nip, $data);
            $data['nip'] = $nip;
            $item = $data;
        }
    ...
    public function show_item()
    {
        if($this->input->server("REQUEST_METHOD") == "POST") {
            ...
            $item = $this->item_model->select_by_id($nip)->row();
    ...
    {
        $data['daftar_item'] = $this->item_model->select_by_id($nip)->row();
    ...
    {
        $this->item_model->delete_item($nip);
        redirect('newitem');
    }
    ...
    {
        $nip = $this->input->post('nip');
        $data['daftar_item'] = $this->item_model->select_by_kode($nip)->result();
        $this->load->view('t-new-item', $data);
    }
